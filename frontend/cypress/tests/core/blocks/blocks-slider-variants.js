context('Blocks Acceptance Tests', () => {
  ...
  it('As editor I can add a full-height variant slider block', () => {
    ...
    cy.get('.blocks-chooser .mostUsed .button.slider')
      .contains('Slider')
      .click({ force: true });
    cy.get('#field-variation').click().type('full-height{enter}');
    cy.get(
    ...
    cy.get('.teaser-item.full-height.has--slider--flagAlign--left').should(
      'have.css',
      'background-color',
      'rgba(0, 0, 0, 0.75)',
    );
  ...
  it('As editor I can add a full-side variant slider block', () => {
    ...
    cy.get('.blocks-chooser .mostUsed .button.slider')
      .contains('Slider')
      .click({ force: true });
    cy.get('#field-variation').click().type('full-side{enter}');
    cy.get(
      '.objectbrowser-field[aria-labelledby^="fieldset-default-field-label-href-0-"] button[aria-label="Open object browser"]',
    ).click();
    ...
    cy.get(
      ...
      .should('have.attr', 'src')
      .and('include', '/document/my-image/');
    cy.get('.teaser-item.full-side.has--slider--flagAlign--left').should(
      'have.css',
      'background-color',
      'rgba(0, 0, 0, 0.75)',
    );
  ...
  it('As editor I can add a side variant slider block', () => {
    ...
    cy.get('.blocks-chooser .mostUsed .button.slider')
      .contains('Slider')
      .click({ force: true });
    cy.get('#field-variation').click().type('side{enter}');
    cy.get(
      '.objectbrowser-field[aria-labelledby^="fieldset-default-field-label-href-0-"] button[aria-label="Open object browser"]',
    ).click();
