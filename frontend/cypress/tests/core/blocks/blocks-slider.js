context('Blocks Acceptance Tests', () => {
  ...
  it('As editor I can add a slider block', () => {
    ...
    cy.get('.align-buttons button[aria-label="Center"]').click();
    ...
    cy.get(
      '.teaser-item-title.fix-width-issue.has--slider--flagAlign--center',
    ).should('have.css', 'background-color', 'rgba(30, 80, 130, 0.85)');
    ...
    cy.get(
      '.teaser-item-title.fix-width-issue.has--slider--flagAlign--left',
    ).should('have.css', 'background-color', 'rgba(0, 0, 0, 0.75)');
  });
});
